...
class Perceptron(object):
    ...
    def sigmoidActivation(self, inActs):
        ...
        """YOUR CODE"""
        a = [1.0] + inActs
        return self.sigmoid(self.getWeightedSum(a))
    ...
    def sigmoidActivationDeriv(self, inActs):
        ...
        """YOUR CODE"""
        a = [1.0] + inActs
        return self.sigmoidDeriv(self.getWeightedSum(a))
...
class NeuralNet(object):
    ...
    def feedForward(self, inActs):
        ...
        for layer in self.layers:
            z = []
            for ptron in layer:
                z.append(ptron.sigmoidActivation(a))
            a = z
            y.append(z)
        return y
    ...
    def backPropLearning(self, examples, alpha):
        ...
        for example in examples:#for each example
            ...
            for outputNum in xrange(len(example[1])):
                ...
                y = example[1][outputNum]   # exact
                yhat = lastLayerOutput[outputNum] # estimate
                error = y - yhat
            ...
            for layerNum in xrange(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in xrange(len(layer)):
                    ...
                    sumWeightDelta = 0
                    for nextNeuronNum in xrange(len(nextLayer)):
                    # Summation of weight[j->k] * delta[k]
                        weightDelta = nextLayer[nextNeuronNum].weights[neuronNum + 1] * deltas[0][nextNeuronNum]
                        sumWeightDelta += weightDelta
                    # gPrime * Summation of weight[j->k] * delta[k]
                    delta = gPrime * sumWeightDelta
                    """FILL IN - delta for this neuron
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxint, startNNet = None):
    ...
    '''
        The test set is a group of inputs and accurate outputs.
        If the neural net is trained, its results will match
        the test set results.
    '''

    for exIn, exOut in examplesTest:
        neuralNetOut = nnet.feedForward(exIn)[-1]
        correct = True
        for exp, ex in zip(neuralNetOut, exOut):
            if round(exp) != ex:
                correct = False
                break
        if correct:
    ...
    testTotal = (testCorrect + testError)
    testAccuracy = (testCorrect * 1.0) / (testTotal * 1.0) #num correct/num total
